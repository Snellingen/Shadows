...
float4 DrawShadowsNoAttenuationPS(float2 TexCoord  : TEXCOORD0) : COLOR0
{
	...
	//float normilizedDistance = floor(range16bit * distance * 2.0f);
	//float normalizedShadowMapDistance = floor(range16bit * shadowMapDistance);

	//float light = normilizedDistance < normalizedShadowMapDistance ? 1:0;
...
float4 DrawShadowsCircleCutPS(float2 TexCoord  : TEXCOORD0) : COLOR0
{
	float2 newCoords = TexCoord - 0.5f;
	...
	if (distance <= 0.5f && distance < distanceMod)
	{
		//we use these to determine which quadrant we are in
		if(abs(nY)<abs(nX))
		{
		...
		//if distance to this pixel is lower than distance from shadowMap, 
		//then we are not in shadow
		light = distance < shadowMapDistance ? 1:0;
	}
...
float4 BlurVerticallyLowCircleCutPS(float2 TexCoord  : TEXCOORD0) : COLOR0
{
	float sum=0;
	float distance = tex2D( inputSampler, TexCoord).b * 0.5f;
...
float4 BlurVerticallyMidCircleCutPS(float2 TexCoord  : TEXCOORD0) : COLOR0
{
	float sum=0;
	float distance = tex2D( inputSampler, TexCoord).b * 0.5f;
...
float4 BlurVerticallyHighCircleCutPS(float2 TexCoord  : TEXCOORD0) : COLOR0
{
	float sum=0;
	float distance = tex2D( inputSampler, TexCoord).b * 0.5f;
...
technique DrawShadowsCircleCut
{
    pass P0
    {          
        VertexShader = compile vs_2_0 FullScreenVS();
        PixelShader  = compile ps_2_0 DrawShadowsCircleCutPS();
    }
}
...
technique BlurVerticallyLowCircleCut
{
    pass P0
    {          
        VertexShader = compile vs_2_0 FullScreenVS();
        PixelShader  = compile ps_2_0 BlurVerticallyLowCircleCutPS();
    }
}

technique BlurVerticallyMidCircleCut
{
    pass P0
    {          
        VertexShader = compile vs_2_0 FullScreenVS();
        PixelShader  = compile ps_2_0 BlurVerticallyMidCircleCutPS();
    }
}

technique BlurVerticallyHighCircleCut
{
    pass P0
    {          
        VertexShader = compile vs_2_0 FullScreenVS();
        PixelShader  = compile ps_2_0 BlurVerticallyHighCircleCutPS();
    }
}
